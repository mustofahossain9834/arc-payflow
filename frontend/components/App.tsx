...
import { Shell28, Brand } from "./shells";
...
const brand: Brand = { name: "Arc Payflow", sub: "Salary on autopilot", emoji: "💵", color: "orange", font: '"Franklin Gothic Medium","Arial Narrow",sans-serif', shape: "rounded-full", hero: "Drip payments", herosub: "DeFi baked right in." };
function Tile({ id, me, pending, send }: { id: bigint; me?: string; pending: boolean; send: (fn: string, args: any[], v?: bigint) => void }) {
  ...
  const done = false;
  return (
    <div className="bg-[var(--card)] border border-[color:var(--cardb)] rounded-[var(--rad)] p-4 space-y-2 hover:border-orange-500/40 transition-colors">
      <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-orange-500/15 grid place-items-center text-lg shrink-0">💵</div>
...
export default function App() {
  ...
  return (<Shell28 brand={brand} tabs={[["earn", "Save"], ["home", "Payroll"], ["swap", "Swap"]]} tab={tab} setTab={setTab}>
    {tab === "home" && <div className="space-y-4">
      <div className="bg-[var(--card)] border border-[color:var(--cardb)] rounded-[var(--rad)] p-6 space-y-3">
        <input value={f.payee} onChange={e=>setF(v=>({...v,payee:e.target.value}))} placeholder="Payee 0x…" type="text" className="w-full bg-[var(--ipt)] border border-[color:var(--iptb)] rounded-[var(--rad)] px-4 py-2.5 text-sm focus:outline-none focus:border-orange-500" />
        <input value={f.rate} onChange={e=>setF(v=>({...v,rate:e.target.value}))} placeholder="Rate / day" type="number" className="w-full bg-[var(--ipt)] border border-[color:var(--iptb)] rounded-[var(--rad)] px-4 py-2.5 text-sm focus:outline-none focus:border-orange-500" />
    ...
    {tab === "earn" && <DefiPanel color="orange" show={["earn"]} note="Liquidity pool + yield, 0.3% fee, no middlemen · Arc." />}
    {tab === "swap" && <DefiPanel color="orange" show={["swap"]} note="Liquidity pool + yield, 0.3% fee, no middlemen · Arc." />}
  </Shell28>);
}